...
const allInside = (m: MeasureInfo) =>
	m.tiles.every((t: TileInfo) => !t.outOfStage && !t.overDock && !t.overTop) &&
...
check(
	...
	!(await evalJS(`document.getElementById("gridBtn").disabled`)),
);
...
check(
	...
	(await evalJS(`zooms.get("aaaa1111").k`)) <= 4.0001,
...
check(
	...
	(await evalJS("zooms.size")) === 0 && (await evalJS("focusId")) === null,
);
...
check(
	...
	await evalJS(`!document.getElementById("gate").hidden && !gateLocked`),
);
...
check(
	...
	(await evalJS(`names.get(myId)`)) === "Gabriel",
	await evalJS("myName"),
);
...
check(
	"the hash follows",
	(await evalJS("location.hash")) === "#trabalho",
